...
int main(int argc, char *argv[]){
  ...
  */
  ...
  for(int i=startEntry;i<endplusEntry;i++){
    ...
    else if(smearRegion==1){
      
    }
    if(debug>=1) { std::cout<<"vtx:--->  "; event->Primaries.begin()->GetPosition().Print(); std::cout<<"------> passed FV ------->  :) :) :) :):) :)"<<std::endl;}
    isHtarget=false;
    brIEntry=i;
    ...
    brVtx[0]=event->Primaries.begin()->GetPosition().X();
    brVtx[1]=event->Primaries.begin()->GetPosition().Y();
    brVtx[2]=event->Primaries.begin()->GetPosition().Z();
    ...
    if(inSTT(event->Primaries.begin()->GetPosition().Vect()) && smearRegion!=1)  smearEvent_STT(); // std::cout<<"have stt event"<<std::endl; //  continue; // smearEvent_STT();
    else if(smearRegion!=0) smearEvent_ECAL();
  }    
